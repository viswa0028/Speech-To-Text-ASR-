...
Original file is located at
    https://colab.research.google.com/drive/18pGwDdSyQu8Y42QztphGQ9DT2PGIMbVq
"""
...
class Decoder(nn.Module):
    ...
    # Ensure x is 2D: [batch_size, seq_len]
      ...
      if hidden is None:
          ...
          # Ensure hidden is a tuple of (hidden, cell)
...
# Debugging: show some examples of tokenized texts
# More detailed tokenization debugging
for i, (filename, indices) in enumerate(list(y_train_dict.items())[:3]):
    ...
    # Print character by character with corresponding token
    print("Character-by-character tokenization:")
    for j, char in enumerate(original_text):
        if j < len(indices) - 2:  # Exclude SOS and EOS tokens
            token_idx = indices[j + 1]  # +1 to skip SOS token
...
if not os.path.exists(encoder_path) or not os.path.exists(decoder_path):
    print(f"Error: Model files not found at specified paths")
    print(f"Encoder path: {encoder_path}")
    print(f"Decoder path: {decoder_path}")
else:
    print("Loading models from Drive...")

    # Load the models - they're already state dictionaries
    encoder_state_dict = torch.load(encoder_path)
    decoder_state_dict = torch.load(decoder_path)

    # 3. Create a compressed model package
    model_data = {
        'encoder_state_dict': encoder_state_dict,
        'decoder_state_dict': decoder_state_dict,
    }

    # 4. Save with highest compression
    compressed_path = '/content/drive/MyDrive/speech_model_data.pt'
    torch.save(model_data, compressed_path, _use_new_zipfile_serialization=True)

    # 5. Further compress using zipfile with highest compression
    zip_path = '/content/drive/MyDrive/compressed_speech_model.zip'
    with zipfile.ZipFile(zip_path, 'w', compression=zipfile.ZIP_DEFLATED, compresslevel=9) as zf:
        zf.write(compressed_path, arcname='speech_model_data.pt')

    # 6. Check the sizes
    original_size_encoder = os.path.getsize(encoder_path) / (1024 * 1024)
    original_size_decoder = os.path.getsize(decoder_path) / (1024 * 1024)
    compressed_size_pt = os.path.getsize(compressed_path) / (1024 * 1024)
    compressed_size_zip = os.path.getsize(zip_path) / (1024 * 1024)

    print(f"Original encoder size: {original_size_encoder:.2f} MB")
    print(f"Original decoder size: {original_size_decoder:.2f} MB")
    print(f"Total original size: {original_size_encoder + original_size_decoder:.2f} MB")
    print(f"Compressed PT file size: {compressed_size_pt:.2f} MB")
    print(f"Compressed ZIP file size: {compressed_size_zip:.2f} MB")

    # Optional: Remove the intermediate .pt file to save space
    if compressed_size_zip < 25:
        print(f"Successfully compressed models to under 25MB!")
        os.remove(compressed_path)
        print(f"Removed intermediate PT file. Final compressed model saved at {zip_path}")
    else:
        print(f"Compression resulted in {compressed_size_zip:.2f} MB, still above 25MB target.")
